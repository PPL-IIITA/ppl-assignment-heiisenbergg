...
import com.ppl_assignment.exception.*;
...
public class q5{
    public ArrayList<couples> matching_algo (ArrayList<boy> boyArrayList1,ArrayList<girl> girlArrayList1) throws FileNotFoundException {
        ArrayList<couples> couplesArrayList = new ArrayList<couples>(10);
        try {
            if (boyArrayList1.size() == 0) {
                throw new ArrayEmpty(1);
            }
            if(girlArrayList1.size() == 0){
                throw new ArrayEmpty(2);
            }
        }catch(ArrayEmpty a){
            a.Message();
            if(a.getType() == 1)
            boyArrayList1 = a.getBoy();
            if(a.getType() == 2)
            girlArrayList1 = a.getGirl();
        }
